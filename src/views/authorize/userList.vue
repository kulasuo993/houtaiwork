...
export default {
  ...
  data() {
    return {
      ...
      EmailRule: /^((([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6}\;))*(([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})))$/,
      passwordRule: /(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[\W_]).{6,16}/,
      list: null,
      list2: null,
      arr: [],
      arr2: [],
      ...
      dialog: {
        ...
        formRule: {
          ...
          email: [
            ...
            {
              validator: (_, value) => {
                if (this.EmailRule.test(value.replace(/\r+|\n+/g, ';'))) {
          ...
          password: [
            ...
            {
              validator: (_, value) => {
                if (this.passwordRule.test(value.replace(/\r+|\n+/g, ';'))) {
                  return Promise.resolve()
                } else {
                  return Promise.reject(new Error('密码不少于6位，必须包含数字和大小写字母和一个特殊字符，特殊字符可包含($,@,#,!,%,*,?,&,.)'))
                }
              }
            }
          ]
