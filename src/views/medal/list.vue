  ...
  <div class="userListPage">
    <div class="table-top">
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        <el-form-item label="">
          <el-select v-model="searchForm.medal_type" clearable size="large" placeholder="请选择勋章类型">
        ...
        <el-form-item label="">
          <el-select v-model="searchForm.is_visible" clearable size="large" placeholder="商品状态">
            <el-option label="已上架" :value="1" />
            <el-option label="未上架" :value="0" />
          </el-select>
        </el-form-item>
        <el-form-item label="">
          <el-select v-model="searchForm.acquire_type" clearable size="large" placeholder="获取类型">
        ...
        <el-form-item label="">
          <el-select v-if="searchForm.acquire_type === '2'" v-model="payType" clearable size="large" placeholder="支付方式">
            <el-option label="创作币" value="coin" />
            <el-option label="萤火币" value="fire" />
          </el-select>
        </el-form-item>
        <el-form-item label="">
          <el-input v-model="searchForm.medal_name" clearable placeholder="请输入勋章名称" />
        </el-form-item>
        <el-form-item>
    ...
    >
      <el-form ref="form" :rules="dialog.formRule" :model="dialog.form" label-width="100px" size="small">
        ...
        <el-form-item label="活动名称" prop="medal_name">
          <el-input v-model="dialog.form.medal_name" />
        </el-form-item>
        <el-form-item label="描述" prop="medal_desc">
          <el-input v-model="dialog.form.medal_desc" type="textarea" />
        </el-form-item>
        <el-form-item label="获取类型" prop="acquire_type">
          <el-select v-model="dialog.form.acquire_type" placeholder="请选择获取类型" style="width: 100%">
            <el-option label="积分兑换" :value="'1'" />
            <el-option label="条件获取" :value="'2'" />
            <el-option label="VIP赠送" :value="'3'" />
          </el-select>
        </el-form-item>
        <el-form-item label="获取条件" prop="acquire_condition">
          <el-input v-model="dialog.form.acquire_condition" type="textarea" />
        </el-form-item>
        <el-form-item label="获取描述" prop="acquire_desc">
          <el-input v-model="dialog.form.acquire_desc" type="textarea" />
        </el-form-item>
        <el-form-item label="是否可见" prop="is_visible">
          <el-radio-group v-model="dialog.form.is_visible">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="是否默认使用" prop="default_used">
          <el-radio-group v-model="dialog.form.default_used">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="是否绝版" prop="is_legacy">
          <el-radio-group v-model="dialog.form.is_legacy">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="排序标识" prop="sort_flag">
          <el-input v-model="dialog.form.sort_flag" />
        </el-form-item>
        <el-form-item label="内链地址" prop="inner_link">
          <el-input v-model="dialog.form.inner_link" />
        </el-form-item>
        <el-form-item label="有效时间" prop="expired_type">
          <el-radio-group v-model="dialog.form.expired_type">
            <el-radio :label="'forever'">永久有效</el-radio>
            <el-radio :label="'since'">固定时间</el-radio>
            <el-radio :label="'fixed'">日期区间</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="dialog.form.expired_type==='since'">
          <el-input v-model="dialog.form.expired_day" />
        </el-form-item>
        <el-form-item v-if="dialog.form.expired_type==='fixed'">
          <el-date-picker
            v-model="dialog.form.expired_time"
            type="datetimerange"
  ...
  methods: {
      ...
      apiMedalList(this.searchForm).then(response => {
        ...
        for (let i = 0; i < this.list.length; i++) {
          ...
          if (this.list[i].acquire_type === '1') {
            if (JSON.parse(this.list[i].acquire_condition)) {
    ...
    onSubmit() {
      new Promise((resolve, reject) => {
        ...
        if (!this.dialog.form.medal_id) {
          const req = JSON.parse(JSON.stringify(this.dialog.form))
          if (req.expired_type === 'forever') {
            req.expired_time = JSON.stringify({
              type: req.expired_type,
              day: 2147483647
            })
          } else if (req.expired_type === 'since') {
            if (req.expired_day === '') {
              this.$message('请输入生效天数', 'error')
              return
            }
            req.expired_time = JSON.stringify({
              type: req.expired_type,
              day: Number(req.expired_day)
            })
          } else if (req.expired_type === 'fixed') {
            if (req.expired_time === '') {
              this.$message({
                message: '请选择开始时间和结束对象',
                type: 'error'
              })
              return
            }
            req.expired_time = JSON.stringify({
              type: req.expired_type,
              start_time: new Date(req.expired_time[0]).getTime() / 1000,
              end_time: new Date(req.expired_time[1]).getTime() / 1000
            })
          }
          delete req.expired_day
          delete req.expired_type
          delete req.medal_id
        // apiMedalCreate(req).then(res =>{
        //   this.$message({
        //     message:"成功创建",
        //     type:'success'
        //   })
